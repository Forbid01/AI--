...
export default function ProServiceSite({ content, theme, assets, business, locale = 'mn' }) {
  ...
  return (
    <div style={{ ...style, '--font-display': displayFamily }}
      ...
      <div aria-hidden className="premium-orb left-[-12%] top-[32rem] h-96 w-96 opacity-10" />

      {/* Nav */}
      <nav className="sticky top-0 z-40 bg-white/80 backdrop-blur-2xl border-b border-[var(--hairline)] shadow-sm shadow-[var(--primary)]/5">
        <div className="mx-auto max-w-6xl px-6 h-16 flex items-center justify-between">
          <a href="#" className="flex items-center gap-2.5">
            <span className="h-9 w-9 rounded-xl bg-gradient-to-br from-[var(--primary)] to-[var(--accent)] grid place-items-center text-white font-bold text-sm shadow-lg shadow-[var(--primary)]/20">
          ...
          <div className="hidden md:flex items-center gap-7 text-sm">
            ...
            {(business?.contactPhone || business?.contactEmail) && (
              <a
                ...
                className="shine px-5 py-2.5 rounded-full bg-[var(--primary)] text-white text-sm font-semibold shadow-lg shadow-[var(--primary)]/20 hover:opacity-90 transition-opacity">
      ...
      {content?.hero && (
        <section className="relative overflow-hidden border-b border-[var(--hairline)]">
          <div aria-hidden className="absolute inset-0 pointer-events-none">
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_78%_18%,color-mix(in_srgb,var(--accent)_18%,transparent),transparent_34%),linear-gradient(135deg,color-mix(in_srgb,var(--primary)_8%,transparent),transparent_45%)]" />
          </div>
          ...
          <div className="relative mx-auto max-w-6xl px-6 py-24 md:py-32 grid md:grid-cols-12 gap-12 items-center">
            <div className="md:col-span-7">
              ...
              <div className="reveal reveal-delay-3 mt-10 flex flex-wrap gap-3">
                {content.hero.ctaPrimary && (
                  <a href="#contact"
                    className="group shine px-7 py-3.5 rounded-full bg-[var(--primary)] text-white font-semibold shadow-lg shadow-[var(--primary)]/25 hover:shadow-xl hover:shadow-[var(--primary)]/30 transition-all inline-flex items-center gap-2">
                    {content.hero.ctaPrimary}
            ...
            <div className="md:col-span-5 reveal reveal-delay-2">
              <div className="relative float-tilt">
                <div aria-hidden className="absolute -inset-6 rounded-[2rem] bg-[var(--accent)]/10 blur-2xl" />
                <div className="premium-frame relative aspect-[4/5] rounded-[1.75rem] overflow-hidden border border-white/40 bg-white/40 backdrop-blur">
                {assets?.hero?.url ? (
                ...
                <div className="absolute top-4 left-4 right-4 flex items-center gap-1.5 rounded-full bg-white/80 backdrop-blur-xl border border-white/60 px-3 py-2 shadow-lg shadow-black/5">
                  <span className="h-2 w-2 rounded-full bg-[#ff5f57]" />
                  <span className="h-2 w-2 rounded-full bg-[#febc2e]" />
                  <span className="h-2 w-2 rounded-full bg-[#28c840]" />
                  <span className="ml-2 truncate text-[10px] font-mono text-[var(--muted)]">{business?.businessName}</span>
                </div>
                <div className="absolute bottom-4 left-4 right-4 rounded-2xl bg-white/90 backdrop-blur-xl border border-white/60 px-4 py-3 shadow-xl shadow-black/10">
          ...
          <div className="mx-auto max-w-6xl px-6">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
              {content.services.map((s, i) => (
                <article key={i}
                  className={`premium-card group rounded-3xl border border-[var(--hairline)] bg-white/85 p-7 hover:border-[var(--accent)]/40 transition-all relative overflow-hidden reveal reveal-delay-${Math.min((i % 4) + 1, 4)}`}>
                  <div aria-hidden
          ...
          <div className="mx-auto max-w-6xl px-6">
            ...
            <div className="grid grid-cols-6 gap-4">
              {content.features.slice(0, 4).map((f, i) => {
                ...
                return (
                  <div key={i}
                    className={`${spans[i]} premium-card rounded-3xl border border-[var(--hairline)] bg-white/85 p-8 hover:border-[var(--accent)]/30 transition-all reveal reveal-delay-${Math.min(i + 1, 4)}`}>
          ...
          <div className="mx-auto max-w-6xl px-6">
            ...
            <div className="grid md:grid-cols-3 gap-4">
              {content.testimonials.map((t, i) => {
                ...
                const hue = (nameHash % 200) + 160;
                return (
                <figure key={i}
                  className={`premium-card rounded-3xl border border-[var(--hairline)] bg-white/85 p-8 transition-shadow reveal reveal-delay-${Math.min(i + 1, 4)}`}>
                  ...
                  <figcaption className="mt-6 flex items-center gap-3">
                    <div
                      ...
                      style={{ background: `linear-gradient(135deg, hsl(${hue} 70% 45%), hsl(${hue + 30} 72% 55%))` }}
                      aria-hidden
                    >
        ...
        <section id="faq" className="py-24 md:py-32 border-b border-[var(--hairline)]">
          <div className="mx-auto max-w-3xl px-6">
            ...
            <div className="space-y-3">
              {content.faq.map((q, i) => (
                <details key={i}
                  className={`premium-card group rounded-2xl border border-[var(--hairline)] bg-white/85 px-6 py-5 open:bg-[var(--primary)]/[0.02] transition-colors reveal reveal-delay-${Math.min(i + 1, 4)}`}>
